...
func (c *Client) Search(ctx context.Context, accountID string, query base.SearchQuery) ([]base.Post, error) {
	...
	for i := 1; i <= query.Limit; i++ {
		id := fmt.Sprintf("dy-%s-%s-%s-%d", keyword, query.SortBy, query.TimeRange, i)
		out = append(out, base.Post{
			ID:       id,
			Title:    fmt.Sprintf("Douyin placeholder result %d for %s (sort=%s time=%s)", i, query.Keyword, query.SortBy, query.TimeRange),
			URL:      fmt.Sprintf("https://www.douyin.com/video/%s", id),
			AuthorID: fmt.Sprintf("dy-author-%d", i),
		})
	}
	return out, nil
}
